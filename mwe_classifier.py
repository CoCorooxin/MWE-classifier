...
class MweClassifier(nn.Module):
    ...
    def forward(self, Xtoks_IDs, deprel):
    ...
    def train_model(self, train_data, test_data, epochs=10, lr=1e-3, batch_size=10, device="cpu", reg=None, split_train=0.8):
        ...
        for e in range(epochs):
            ...
            for X_toks, deprel, Y_gold in tqdm(train_loader):
                # print(x.shape)
                optimizer.zero_grad()
    ...
    def predict(self, string):
        ...
        with torch.no_grad():
            for X_toks, deprel, Y_gold in tqdm(test_loader):
                logprobs = self.forward(X_toks,deprel)
                # Decode the best tag sequence using Viterbi decoding
                best_path = model.crf.decode(output)

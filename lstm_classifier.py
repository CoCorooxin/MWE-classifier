...
class MweLSTM(nn.Module):
    ...
    def load(modelfile,name, toks_vocab, tags_vocab, embsize, hidden_size, drop_out,  pretrained = False, device="cpu"):
        ...
        model      = MweLSTM(name, toks_vocab,tags_vocab, embsize, hidden_size, drop_out, pretrained, device)
...
if __name__ == '__main__':
    ...
    train   = LstmDataset(config["TRAIN"], isTrain = True)
    test    = LstmDataset(config["TEST"])
    dev     = LstmDataset(config["DEV"])
    ...
    model       = MweLSTM(config['NAME'], toks_vocab, tags_vocab, embsize,hidsize, dropout, pretrained = pretrain, device = device)
